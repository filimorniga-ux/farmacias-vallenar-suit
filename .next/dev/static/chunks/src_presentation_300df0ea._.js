...
"[project]/src/presentation/hooks/useCashSession.ts [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const useCashSession = ()=>{
    _s();
    const { currentShift, salesHistory, cashMovements } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$presentation$2f$store$2f$useStore$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["usePharmaStore"])();
    const metrics = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "useCashSession.useMemo[metrics]": ()=>{
            if (!currentShift) {
                return null;
            }
            // 1. Filter Data for Current Shift
            const shiftSales = salesHistory.filter({
                "useCashSession.useMemo[metrics].shiftSales": (s)=>s.timestamp >= currentShift.start_time && (!currentShift.end_time || s.timestamp <= currentShift.end_time)
            }["useCashSession.useMemo[metrics].shiftSales"]);
            const shiftMovements = cashMovements.filter({
                "useCashSession.useMemo[metrics].shiftMovements": (m)=>m.shift_id === currentShift.id
            }["useCashSession.useMemo[metrics].shiftMovements"]);
            // 2. Process Sales by Method
            const cashSalesItems = shiftSales.filter({
                "useCashSession.useMemo[metrics].cashSalesItems": (s)=>s.payment_method === 'CASH'
            }["useCashSession.useMemo[metrics].cashSalesItems"]);
            const cardSalesItems = shiftSales.filter({
                "useCashSession.useMemo[metrics].cardSalesItems": (s)=>s.payment_method === 'DEBIT' || s.payment_method === 'CREDIT'
            }["useCashSession.useMemo[metrics].cardSalesItems"]);
            const transferSalesItems = shiftSales.filter({
                "useCashSession.useMemo[metrics].transferSalesItems": (s)=>s.payment_method === 'TRANSFER'
            }["useCashSession.useMemo[metrics].transferSalesItems"]);
            const cashSales = {
                count: cashSalesItems.length,
                total: cashSalesItems.reduce({
                    "useCashSession.useMemo[metrics]": (sum, s)=>sum + s.total
                }["useCashSession.useMemo[metrics]"], 0),
                items: cashSalesItems
            };
            const cardSales = {
                count: cardSalesItems.length,
                total: cardSalesItems.reduce({
                    "useCashSession.useMemo[metrics]": (sum, s)=>sum + s.total
                }["useCashSession.useMemo[metrics]"], 0),
            ...
            const transferSales = {
                ...
                total: transferSalesItems.reduce({
                    "useCashSession.useMemo[metrics]": (sum, s)=>sum + s.total
                }["useCashSession.useMemo[metrics]"], 0),
                items: transferSalesItems
            };
            const totalSales = shiftSales.reduce({
                "useCashSession.useMemo[metrics].totalSales": (sum, s)=>sum + s.total
            }["useCashSession.useMemo[metrics].totalSales"], 0);
            // 3. Process Movements
            const inflows = shiftMovements.filter({
                "useCashSession.useMemo[metrics].inflows": (m)=>m.type === 'IN' && m.is_cash
            }["useCashSession.useMemo[metrics].inflows"]);
            const outflows = shiftMovements.filter({
                "useCashSession.useMemo[metrics].outflows": (m)=>m.type === 'OUT' && m.is_cash
            }["useCashSession.useMemo[metrics].outflows"]);
            const otherIncomes = {
                count: inflows.length,
                total: inflows.reduce({
                    "useCashSession.useMemo[metrics]": (sum, m)=>sum + m.amount
                }["useCashSession.useMemo[metrics]"], 0),
                items: inflows
            };
            const expenses = {
                count: outflows.length,
                total: outflows.reduce({
                    "useCashSession.useMemo[metrics]": (sum, m)=>sum + m.amount
                }["useCashSession.useMemo[metrics]"], 0),
                items: outflows
            };
...
_s(useCashSession, "pIjk+XLYH5G71epXEUH1MQ1Keoc=", false, function() {
    return [
...
//# sourceMappingURL=src_presentation_300df0ea._.js.map