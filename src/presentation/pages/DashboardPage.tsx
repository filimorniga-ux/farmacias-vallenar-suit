...
const ModuleCardSkeleton = () => (
    <div className="h-24 bg-slate-100 rounded-2xl animate-pulse flex flex-col items-center justify-center gap-2">
...
const DashboardPage: React.FC = () => {
    ...
    const dashboardData = useMemo(() => {
        ...
        return {
            totalSales: dashboardStats.todaySales || 0,
            transactionCount: dashboardStats.transactionCount || 0,
            inventoryValue: dashboardStats.totalInventoryValue || 0,
            ...
            santiagoSales: dashboardStats.santiagoSales || 0,
            colchaguaSales: dashboardStats.colchaguaSales || 0,
    ...
    const handleEmployeeSelect = (employee: EmployeeProfile) => {
        // Boost Mode: Pre-fetch data specific to user role if needed in future
        prefetchDashboard();
    ...
    const FinancialCard = ({ title, value, icon: Icon, gradient, shadowColor, trend }: any) => (
        <motion.div
            ...
            className={`bg-gradient-to-br ${gradient} p-5 rounded-2xl shadow-lg shadow-${shadowColor}-500/30 text-white relative overflow-hidden group hover:scale-105 transition-transform duration-300`}
        >
    ...
    const ModuleCard = ({ title, icon: Icon, gradient, shadowColor, route }: any) => (
        <motion.button
            ...
            className={`flex flex-col items-center justify-center p-4 bg-gradient-to-br ${gradient} rounded-2xl shadow-lg shadow-${shadowColor}-500/30 border border-white/10 hover:shadow-2xl hover:shadow-${shadowColor}-500/40 hover:scale-105 transition-all duration-300 h-24 group relative overflow-hidden w-full`}
        >
                ...
                <div className="max-w-4xl mx-auto flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            {isLocationMenuOpen && (
                                <>
                                    <div className="fixed inset-0 z-40 cursor-default" onClick={() => setIsLocationMenuOpen(false)}></div>
                        ...
                        <button onClick={handleRefresh} className={`p-2 rounded-full hover:bg-slate-100 transition-colors ${isRefetching ? 'animate-spin text-cyan-600' : 'text-slate-400'}`}>
                            <RefreshCw size={20} />
                        </button>
                    </div>
                </div>
                    ...
                    isQueryLoading ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-8">
                            <FinancialCard
                                ...
                                value={new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP' }).format(dashboardData.totalSales)}
                                icon={Wallet}
                            ...
                            <FinancialCard
                                ...
                                value={new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', notation: "compact" }).format(dashboardData.inventoryValue)}
                                icon={Building2}
                ...
                <section>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {isQueryLoading ? (
                            ...
                            <>
                                <ModuleCard title="Ventas" icon={ShoppingCart} gradient="from-emerald-400 to-cyan-600" shadowColor="cyan" route="/pos" />
                                <ModuleCard title="Bodega" icon={Truck} gradient="from-orange-400 to-pink-600" shadowColor="orange" route="/warehouse" />
                                <ModuleCard title="Admin" icon={Building2} gradient="from-indigo-500 to-purple-600" shadowColor="indigo" route="/settings" />
                                <ModuleCard title="Reportes" icon={BarChart3} gradient="from-blue-400 to-indigo-600" shadowColor="blue" route="/reports" />
                            </>
                        )}
                    </div>
                </section>
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-4 mb-6 max-h-60 overflow-y-auto">
                                        {filteredEmployees.length > 0 ? (
                                            ...
                                            <div className="col-span-2 text-center py-8 text-slate-400"><p>No hay usuarios disponibles</p></div>
                                        )}
                                    </div>
                                    <button onClick={() => setIsLoginModalOpen(false)} className="w-full py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100 transition">Cancelar</button>
                                </>
                            ) : (
                                <>
                                    <div className="text-center mb-8">
                                    ...
                                    <form onSubmit={handleLogin} className="space-y-6">
                                        <div>
                                            <label className="block text-center text-sm font-bold text-slate-600 mb-2">Ingresa tu PIN</label>
                                            <motion.input
                                                key={error}
