...
const KanbanColumn: React.FC<KanbanColumnProps> = ({
    title, status, color, icon: Icon, direction,
    purchaseOrders, suppliers, user,
    onEditOrder, onReceiveOrder, removePurchaseOrder, updatePurchaseOrder
}) => {
...
const SupplyKanban: React.FC<SupplyKanbanProps> = ({ onEditOrder, onReceiveOrder, direction = 'col' }) => {
    ...
    return (
        <div className={`flex flex-1 gap-4 ${direction === 'row' ? 'overflow-x-auto pb-4' : 'flex-col overflow-y-auto pr-2 scrollbar-hide'}`}>
            <KanbanColumn title="Borradores" status="DRAFT" color="text-slate-600" icon={Package} {...sharedProps} />
            <KanbanColumn title="Aprobadas" status="APPROVED" color="text-amber-600" icon={CheckCircle} {...sharedProps} />
            <KanbanColumn title="En Camino" status="SENT" color="text-blue-600" icon={Truck} {...sharedProps} />
            <KanbanColumn title="Recibidas" status="RECEIVED" color="text-green-600" icon={CheckCircle} {...sharedProps} />
        </div>
    );
};
