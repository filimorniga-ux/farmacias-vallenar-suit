...
// =====================================================
// FUNCIÓN: FIND BEST BATCH (FIFO)
// =====================================================

/**
 * Encuentra el mejor lote para descontar stock (FIFO).
 * Prioriza fecha de caducidad más próxima (si tiene) o fecha de creación.
 * 
 * @param sku - SKU del producto
 * @param locationId - ID de la ubicación
 * @param minQuantity - Cantidad mínima requerida (para validar stock suficiente)
 */
export async function findBestBatchSecure(
    ...
    try {
        ...
        const res = await pool.query(`
            SELECT 
                id, quantity_real, expiry_date, sale_price 
            FROM inventory_batches
            WHERE location_id = $1 
            AND sku = $2 
            AND quantity_real >= $3
            ...
            ORDER BY 
                expiry_date ASC NULLS LAST, -- Prioridad: Vence pronto
                created_at ASC              -- Prioridad: Más antiguo (FIFO)
            LIMIT 1
        `, [locationId, sku, minQuantity]);

        if (res.rows.length === 0) {
            return { success: false, error: 'No hay stock suficiente o disponible' };
        }
        ...
        return {
            ...
            batch: {
                id: row.id,
                quantity: Number(row.quantity_real),
        ...
        logger.error({ error, sku, locationId }, 'Error finding best batch');
        return { success: false, error: 'Error consultando stock' };
    }
}
