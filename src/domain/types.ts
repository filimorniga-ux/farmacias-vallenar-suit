...
export interface PurchaseOrder {
    ...
    destination_location_id: string; // Alias for UI, derived from warehouse
    location_id?: string; // Optional real column if doing joins, but not in PO table natively
    target_warehouse_id: string; // The primary foreign key to identify where stock arrives
    created_at: number;
    sent_at?: number; // When sent to supplier
    received_at?: number; // When fully received
    status: POStatus;
    items: PurchaseOrderItem[];
    total_estimated?: number; // Optional, might not be in DB
    is_auto_generated?: boolean; // Virtual/Optional
    generation_reason?: 'LOW_STOCK' | 'FORECAST' | 'MANUAL'; // Virtual/Optional
    notes?: string;
}
