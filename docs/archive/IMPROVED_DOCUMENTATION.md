...
*   **Arquitectura:** Functional Core / Imperative Shell en `src/domain/logic/purchasingAgent.ts`, operando offline-first sobre Zustand. TypeScript estricto para cálculos financieros y de stock sin errores de redondeo.
*   **Ciclo cognitivo (4 fases):**
    1.  **Velocity real:** VDP de 30 días; si hay <7 días de historia, cae en `avg_daily_sale` manual del producto.
    2.  **Gap Analysis:** `stock_objetivo = VDP * días_cobertura (15 por defecto) + safety + ventas en lead time`; resta stock en tránsito para evitar doble compra.
    3.  **Supplier matching:** Usa `preferredSupplierId` cuando existe o mapea categoría→proveedor; extensible a costo histórico y lead time.
    4.  **Clusterización:** Agrupa déficits por proveedor y genera OC en estado `SUGGESTED` con costo total y nota explicativa.
*   **Mejoras estratégicas:** Considera lead time de proveedor, ignora lotes en `QUARANTINE`/`EXPIRED`, redondea con `Math.ceil` para logística, y documenta el criterio de cobertura en la nota de cada OC.
*   **Integración UI:** Botón “Ejecutar Agente IA” en `SupplyChainPage.tsx` que llena la columna "Sugerencias IA" del Kanban; al mover a `DRAFT`, queda bajo control humano antes de ordenar.
