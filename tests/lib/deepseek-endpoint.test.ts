...
import { getDeepSeekInternalEndpoint, resolveDeepSeekOcrEndpoint } from '@/lib/ai/deepseek-endpoint';
...
describe('resolveDeepSeekOcrEndpoint', () => {
    ...
    it('usa ruta interna cuando no hay endpoint explícito', () => {
        delete process.env.AI_DEEPSEEK_OCR_ENDPOINT;
        delete process.env.NEXT_PUBLIC_APP_URL;
        process.env.APP_URL = 'https://farmaciasvallenarsuit.cl';

        const endpoint = resolveDeepSeekOcrEndpoint(null);
        expect(endpoint).toBe('https://farmaciasvallenarsuit.cl/api/ai/deepseek-ocr');
    });

    it('prioriza APP_URL para la ruta interna canónica', () => {
        process.env.APP_URL = 'https://app.example.com';
        process.env.NEXT_PUBLIC_APP_URL = 'https://public.example.com';

        expect(getDeepSeekInternalEndpoint()).toBe('https://app.example.com/api/ai/deepseek-ocr');
    });
});
