...
test.describe('Authentication', () => {
    test.beforeEach(async ({ page }) => {
        ...
        // Handle Context Selection if present (New Flow)
        const contextSelectionTitle = page.locator('text=¿Dónde inicias turno hoy?');
        if (await contextSelectionTitle.isVisible()) {
            // Select the first available location
            const selectButton = page.locator('button').filter({ hasText: 'Seleccionar' }).first();
            if (await selectButton.isVisible()) {
                await selectButton.click();
                // Wait for reload and cookie setting
